...
# --- 2. FUNÇÕES MATEMÁTICAS (GAMMA + VANNA) ---
...
def get_gamma_data_v2(ticker_symbol):
    try:
        ...
        for df, is_put in [(calls, False), (puts, True)]:
            ...
            df['GEX'] = res.apply(lambda x: x[0]) * df['openInterest'] * 100 * S * (1 if not is_put else -1)
            df['VEX'] = res.apply(lambda x: x[1]) * df['openInterest'] * 100 * (1 if not is_put else -1)
...
if not calls_data.empty:
    ...
    c1.metric("Net GEX", f"{net_gex_total:.2f}M", delta_color="normal" if net_gex_total > 0 else "inverse")
    ...
    fig_hist = go.Figure()
    fig_hist.add_trace(go.Bar(x=calls_data['strike'], y=calls_data['GEX'], name='Calls', marker_color='#00ffcc', customdata=calls_data['forca'], hovertemplate="GEX: %{y:,.0f}<br>Força: %{customdata:.2f}%<extra></extra>"))
    fig_hist.add_trace(go.Bar(x=puts_data['strike'], y=puts_data['GEX'], name='Puts', marker_color='#ff4b4b', customdata=puts_data['forca'], hovertemplate="GEX: %{y:,.0f}<br>Força: %{customdata:.2f}%<extra></extra>"))
    ...
    fig_vanna.update_layout(template="plotly_dark", xaxis=dict(range=[current_price * 0.96, current_price * 1.04]), showlegend=False)
