...
if not calls.empty:
    ...
    cor_pw = "#ff4b4b" # Put Wall é tradicionalmente alerta de suporte
    cor_cw = "#00ffcc" # Call Wall é teto de resistência
    
    label_status = "SUPRESSÃO" if net_gex > 0 else "EXPANSÃO"

    st.write(f"### {datetime.now().strftime('%b %d, %Y')}")
    st.markdown(f"<h1 style='color: white; font-size: 55px;'>{label_status}</h1>", unsafe_allow_html=True)
    ...
    # Injeção de cores nas métricas (Verde se Positivo / Vermelho se Negativo)
    st.markdown(f"""
        <style>
        ...
        div[data-testid="column"]:nth-child(4) [data-testid="stMetricValue"] {{ color: {cor_pw} !important; }}
        div[data-testid="column"]:nth-child(5) [data-testid="stMetricValue"] {{ color: white !important; }}
        </style>
    """, unsafe_allow_html=True)
    ...
    # --- 4. ABAS ---
    tab_price, tab_gex, tab_vanna = st.tabs(["📈 Preço", "📊 Gamma Profile", "🌊 Vanna Exposure"])

    with tab_gex:
        st.subheader("📊 Gamma Profile com Spot & Call Wall")
        ...
        # ADICIONADO: Linhas verticais no gráfico de Gamma
        fig_g.add_vline(x=spot, line_color="yellow", line_dash="dash", annotation_text=f"SPOT: ${spot:.2f}")
        fig_g.add_vline(x=call_wall, line_color="#00ffcc", line_dash="dot", annotation_text="Call Wall")
        fig_g.add_vline(x=put_wall, line_color="#ff4b4b", line_dash="dot", annotation_text="Put Wall")

        fig_g.update_layout(template="plotly_dark", barmode='relative', height=600, hovermode="x unified")
    ...
    with tab_vanna:
        ...
        fig_v.add_trace(go.Scatter(x=x_vals, y=[y if y > 0 else None for y in y_vals], mode='lines+markers', name='Vanna (+)', line=dict(color='#00ffcc', width=3)))
        fig_v.add_trace(go.Scatter(x=x_vals, y=[y if y < 0 else None for y in y_vals], mode='lines+markers', name='Vanna (-)', line=dict(color='#ff4b4b', width=3)))
